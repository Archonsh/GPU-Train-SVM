...
int readModel(const char *filename, float **alpha, float **supportVectors, int *nSVOut, int *nDimOut, Kernel_params *kp,
              float *p_class1Label, float *p_class2Label) {
    ...
    for (int sv = 1; sv < nSV; sv++) {
        ...
        do {
            currentCharacter = fgetc(input);
                 ...
                 (currentCharacter != '-') && (currentCharacter != 'e') && (currentCharacter != 'E') &&
                 (currentCharacter >= 0));
        ungetc(currentCharacter, input);
...
int readSvm(const char *filename, float **p_data, float **p_labels, int *p_npoints, int *p_dimension,
    ...
    char firstLine = 1;
    do {
        c = fgetc(inputFilePointer); //Get character from stream
        switch (c) {
            case '\n':
                npoints++;
                firstLine = 0;
            ...
            if (validCharacter > 0) {
                if ((c == ' ') || (c == '\n')) {
                    *(bufferPointer) = 0;
                    ...
                    if (parsingLabel > 0) {
                        labels[i] = value;  //TODO: change it to support sparse storage
                        parsingLabel = 0;
                    } else {
                        data[currentDim * npoints + i] = value; //column major storage
                        if (p_transposed_data != NULL) {
                            transposed_data[i * dimension + currentDim] = value; //row major storage
                        }
                        currentDim++;
                    }
                    validCharacter = 0;
                    bufferPointer = stringBuffer; //归零
                } else {
                    *(bufferPointer) = c;
                    bufferPointer++;
                }
...
void printModel(const char *outputFile, Kernel_params kp, float *alpha, float *labels, float *data, int nPoints,
