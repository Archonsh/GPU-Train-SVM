 ...
 *
 * Reference:
 *   Cheng, J., & McKercher, T. (2014). Shared Memory and Constant Memory. In M. Grossman (Ed.), Professional CUDA C programming (pp. 239-249). Indianapolis, IN: Wrox.
 */
...
void transposeMatrix(float* in, float* out, int nrows, int ncols) {
    ...
    CHECK(cudaMemcpy(d_A, in, nBytes, cudaMemcpyHostToDevice));
    ...
    CHECK(cudaMemcpy(out, d_C, nBytes, cudaMemcpyDeviceToHost));

    return ;
}

// Examples on how to use different transpose matrix functions 
